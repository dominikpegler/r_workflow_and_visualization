...
names(df)
```
## Task 3: What is the mean of the variable "DominancePicture"? Here you can use different approaches to get the solution!
```{r}
...
## Task 5: Add a new column to the dataset. Use the cbind() function for this. The new column should be all numbers from 1 to 40 (extra point: numbers are in a randomized order). For help on the function call ?cbind or use Google ;)
```{r}
df$New_Col <- cbind(sample(seq(1:40)))
str(df$New_Col)
```

## Task 6: The function duplicated() is used to detect duplicates (e.g. same row is twice in your data). Interpret the output in one sentence when applying it to the data set
