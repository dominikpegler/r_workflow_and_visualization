...
## 1) Make a scatterplot of hwy (y-axis) vs. cyl (x-axis) from the dataset mpg (from tidyverse). 
...
## 2) Add jitter to the scatterplot from example (1) to avoid overplotting. 
...
## 3) Can you avoid plotting the empty category of 7 cylinders in examples (1) and (2)? (Me: And differentiate each class by color.)
...
## 4) Use boxplots for each level of cyl to visualize the distribution of hwy. (Hint: cyl should be treated as a factor using as.factor().) 
...
## 5) Overlay the plots of example (2/3) and (4). Use a different color for the points from plot (2). (Hint: Plot the scatterplot over the boxplots to make all the points visible.)
